class Particle {
  ...
  float getPositionY(){
   ...
   void attract(float x,float y) {
    ...
    PVector particle = new PVector(0,0);
    particle.x = getPositionX();
    particle.y = getPositionY();
    float X = abs(bodyVec.x - particle.x);
    float Y = abs(bodyVec.y - particle.y);
    float D = sqrt(X*X + Y*Y);
    D = D/100;
    // Then, scale the vector to the specified force
    worldTarget.normalize();
    worldTarget.mulLocal(D);
